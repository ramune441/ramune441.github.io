...
| `/ashiato-maker/live-b.png` `live-c.png` | 画像 | - | no | **拡張機能を実際に稼働させた**Pairs画面キャプチャ(2400x1500)。実パネル(日本語化/停止ボタン/進捗)・訪問中バッジ・通知トースト・訪問済マーク入り。写真は実写真ぼかし。promoのデモシーン背景(b=稼働中→c=訪問済へクロスフェード) |
| `/ashiato-maker/promo.html` | 動画用 | noindex | no | 録画専用。6シーン構成（フック「気づかれていないだけ」→足あとの仕組み→**実稼働画面デモ**(`live-b/c.png`+訪問リング+「1時間に100人以上」)→「足あとを残した分だけいいねは返ってくる」→**機能6グリッド「機能は、ぜんぶ入り。しかも、ずっと無料。」**→CTA「あなたは、ボタンを押すだけ。今日から気づかれる側へ」）・約42.5秒 |
