...
| `/ashiato-maker/promo.html` | 動画用 | noindex | no | 録画専用。6シーン構成（フック「気づかれていないだけ」→足あとの仕組み→**実稼働デモ**(①開始→②`live-seq-1..6`の高速連続で新しい会員に次々足あと[実オーバーレイvisit差し込み＋足あとスタンプ]「新しい会員だけに/訪問済みは自動スキップ・重複しません」→③live-fullで1画面まるごと訪問済み＋「1時間に100人以上」)→「足あとを残した分だけいいねは返ってくる」→**機能6グリッド「機能は、ぜんぶ入り。しかも、ずっと無料。」**→CTA「あなたは、ボタンを押すだけ。今日から気づかれる側へ」）・約43秒。※偽の訪問リング/偽カウンターは使わず実キャプチャのみ |
