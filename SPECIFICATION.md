...
| `/subduo/` | LP | index | yes | Chrome拡張（SubDuo・動画配信の二言語字幕）。**対応は Prime Video / YouTube / Netflix / Disney+ の4サービス**で、設定はサイトごとに独立（プリセット機能は無い）。ダークテーマ。`i18n-lp.js` で拡張UIと同じ12言語対応（en=DOM既定、ja/zh/zh-TW/ko/es/fr/de/it/pt/ru/hi）＋言語セレクター（`?lang=` > localStorage > ブラウザ言語）。**HTMLの既定文（英語）は必ず `i18n-lp.js` の en 辞書と同じ文字列にする**（実行時に上書きされるため、ずれるとクローラだけが古い文言を見る）。デモは実画面ではなく架空作品のプレイヤーモック（ヒーローは cue を3.6秒周期で切替）。YouTube 埋め込み（`#promo-video`）は `LP_VIDEO_IDS` により表示言語の動画に自動切替。ウェブストアCTAは `SUBDUO_STORE_URL` 定数（掲載ID `ngifacacaecobbekalngmocklcgkoigo`）から全ボタンに配る |
| `/subduo/contact.html` | 問い合わせ | index | yes | 自前フォーム（自己完結型・インライン12言語辞書＋言語セレクター、localStorage キーは LP と同じ `lp_lang`）。「どの配信サービスか」選択（Prime Video/YouTube/Netflix/Disney+/Other → `app` フィールド）つき。`?lang=` で拡張の UI 言語を引き継ぐ。本文は `maxlength=5000`（超えるとサーバが 400 で弾くため） |
| `/subduo/uninstall.html` | アンケート | noindex | no | アンインストール理由アンケート。`chrome.runtime.setUninstallURL` の遷移先。**12言語対応**（他プロダクトの同ページは単一言語）。拡張がクエリで渡す匿名診断（`v`/`days`/`err`）を hidden `diagnostics` に集約し、`lang` は表示言語と `language` 欄に反映。理由の選択肢は英語カノニカル値のまま送り表示のみ翻訳。詳細欄は必須 |
| `/subduo/privacy-policy.html` | 規約 | index | yes | プライバシーポリシー（英語／日本語の切替ボタン、`?lang=ja` で日本語）。完全ローカル動作・権限は storage / scripting と対応4サイトのみ（v1.0.2 で `tabs` は外れた。ストア説明文・LP・ポリシーは manifest の permissions と一致させる）。YouTube の字幕一覧取得は Cookie を送らない旨も明記 |
| `/subduo/promo.html` | 動画用 | noindex | no | 録画専用（1920x1080、6シーン: イントロ→Before/After→3ステップ→デモ4本[二言語字幕/ポップアップで言語選択/ドラッグ・ホイール/右クリックパネル]→機能6グリッド→CTA、約36.5秒）。**文言は独自辞書を持たず `i18n-lp.js` の `LP_MESSAGES` をそのまま使う**（`?lang=` で12言語、en=DOM既定）。デモUIは index.html のプレイヤー／ポップアップ／パネルのモックを拡大したもの |
| `/subduo/promo[-lang].webm` | 動画 | - | no | Chrome Web Store 用（VP9/yuv420p/1080p/30fps/39.5秒・無音）。en=`promo.webm`、翻訳版=`promo-{ja,zh,zh-TW,ko,es,fr,de,it,pt,ru,hi}.webm`。`record-promo.mjs` が **3840x2160 ビューポート + `html{zoom:2}`** で `page.screencast()` 録画（deviceScaleFactor=2 では screencast が CSS px のまま 1080p になる）→実測係数（wall-clock ÷ raw実尺）で setpts→lanczos 縮小。録画開始から最初のフレームまで ~0.7s 遅れるぶん尺に余裕を持たせる |
| `/subduo/promo[-lang].mp4` | 動画 | - | no | YouTube 掲載用 4K（H.264 High/yuv420p/3840x2160/30fps + AAC 192k BGM `mp3/271_BPM110.mp3` 音量0.85・0.6s フェードイン・2s フェードアウト）。掲載タイトル/説明文は `subduo/youtube.json`。**YouTube 公開済み（チャンネル: Just One More Animal）**: en=Bt77OCrpUnk / ja=Jp1TjBxZxto / zh=eoimxCj4PRs / zh-TW=2VWXTUd_mV0 / ko=QppIza5SWs0 / es=VkRPztFOxeE / fr=tRGVESBz834 / de=1t2N-RPqfM8 / it=mDftA2N5ZnI / pt=8OrAuDa_F54 / ru=1l8z_GPCpL8 / hi=rfBC1whtjyU。ストア掲載情報の「ローカライズ版プロモーション動画」に言語ごとの `https://www.youtube.com/watch?v=<ID>`、「全言語向け」に en を設定済み（掲載情報は審査待ちの間ロックされ保存できない。編集するには ⋮ → 審査をキャンセル → 保存 → 再送信） |
...
- **フィールド**: `product`（FormPilot/SendReady/SubDuo/Ashiato Maker/Lovent/Ripen/Charmly/Cookpal。**許可リスト外は 400 `Invalid product`** なので、新プロダクトは worker の `PRODUCTS` に足して手動デプロイするまで送信できない）/ `category`（Bug Report/Feature Request/Other、contact時必須）/ `reasons[]`（uninstall時必須・英語カノニカル値）/ `reasonOther` / `content`（5000文字まで）/ `email`（任意）/ `app`（ashiato=マッチングアプリ名、subduo=配信サービス名）/ `device` `osVersion`（Loventアプリが `?device=&os=` で引き渡し）/ `attachment`（任意、画像・動画・PDF・テキスト、25MBまで → R2 保存しメールにDLリンク記載）/ `language` / `botcheck`（ハニーポット）
